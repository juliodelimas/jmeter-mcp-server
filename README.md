...
| Distributed execution (master-slave) | Run `execute_test_plan` across multiple JMeter injector nodes via `-R`, orchestrated by this server, instead of a single local process | JMeter supports distributed mode, but wiring up remote engines, RMI ports, and matching JMeter versions across machines is entirely manual today — nobody sets this up for a quick test |
...
| `orchestrate_distributed_run` | Coordinates multiple JMeter injectors (master-slave) and merges every node's `.jtl` into a single aggregated report | Builds on top of distributed execution above — JMeter supports `-R`, but turning that into one coherent report is still manual work |
